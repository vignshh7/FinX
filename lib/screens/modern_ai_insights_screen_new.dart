...
class _ModernAIInsightsScreenState extends State<ModernAIInsightsScreen>
  ...
  Widget _buildCompleteAIAnalysisSection(ExpenseProvider provider, bool isDark) {
    ...
    final aggregation = analysis['aggregation'] is Map
      ? analysis['aggregation'] as Map
      : (analysis['aggregation'] is List
        ? _normalizeAggregation({'trend': analysis['aggregation']})
        : null);
    ...
    final adviceList = advice != null
      ...
      : ((insightsMap?['budget_recommendations'] as List?) ?? <dynamic>[]);
    final insightTexts = insightsMap != null
        ? ((insightsMap['insights'] ?? []) as List)
        : <dynamic>[];

    final explainability = insightsMap?['explainability'] is Map
      ? insightsMap!['explainability'] as Map
      : null;
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          // Insights from AI
          if (insightTexts.isNotEmpty || explainability != null) ...[
            const SizedBox(height: 20),
            ...
            if (explainability != null) ...[
              if ((explainability['what'] ?? '').toString().isNotEmpty)
                Padding(
                  padding: const EdgeInsets.only(bottom: 8),
                  child: Row(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Icon(Icons.insights, size: 16, color: FintechColors.warningColor),
                      const SizedBox(width: 8),
                      Expanded(
                        child: Text(
                          explainability['what'].toString(),
                          style: FintechTypography.bodySmall.copyWith(color: statColor, height: 1.4),
                        ),
                      ),
                    ],
              ...
              if ((explainability['why'] ?? '').toString().isNotEmpty)
                Padding(
                  padding: const EdgeInsets.only(bottom: 8),
                  child: Row(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Icon(Icons.help_outline, size: 16, color: FintechColors.accentTeal),
                      const SizedBox(width: 8),
                      Expanded(
                        child: Text(
                          explainability['why'].toString(),
                          style: FintechTypography.bodySmall.copyWith(color: statColor, height: 1.4),
                        ),
                      ),
                    ],
          ...
          if (adviceList.isNotEmpty) ...[
            ...
            ...adviceList.take(3).map((item) {
              final text = item is Map
                  ? (item['message'] ?? item['reason'] ?? item['advice'] ?? item['suggestion'] ?? item.toString())
                  : item.toString();
              return Padding(
                padding: const EdgeInsets.only(bottom: 8),
                child: Row(
  ...
  Widget _buildSpendingAggregationSection(ExpenseProvider provider, bool isDark) {
    final rawAggregation = provider.spendingAggregation ?? _buildLocalAggregation(provider);
    final aggregation = _normalizeAggregation(rawAggregation);
  ...
  Map<String, dynamic>? _normalizeAggregation(Map<String, dynamic>? aggregation) {
    if (aggregation == null) return null;

    // Analytics endpoint may return { months, trend: [{month,total}] }
    if (aggregation['monthly_totals'] == null && aggregation['trend'] is List) {
      final monthlyTotals = <String, double>{};
      for (final item in (aggregation['trend'] as List)) {
        if (item is Map) {
      ...
      final overall = monthlyTotals.values.fold<double>(0.0, (sum, amount) => sum + amount);
      return {
        ...aggregation,
        'monthly_totals': monthlyTotals,
        'overall_total': aggregation['overall_total'] ?? overall,
        'months_analyzed': aggregation['months_analyzed'] ?? monthlyTotals.length,
        'average_monthly': aggregation['average_monthly'] ?? (monthlyTotals.isNotEmpty ? overall / monthlyTotals.length : 0.0),
      };
    }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 24),
          ..._buildEnhancedInsightItems(insights, isDark),
  ...
  List<Widget> _buildEnhancedInsightItems(Map<String, dynamic> insights, bool isDark) {
    final widgets = <Widget>[];

    if (insights['insights'] is List) {
    ...
    final explainability = insights['explainability'] is Map
        ? insights['explainability'] as Map
        : null;
    if (explainability != null) {
      for (final key in ['what', 'why', 'next_steps']) {
        final text = (explainability[key] ?? '').toString();
        if (text.isNotEmpty) {
          widgets.add(
            Padding(
              padding: const EdgeInsets.only(bottom: 12),
              child: _buildInsightItem({'title': key.replaceAll('_', ' ').toUpperCase(), 'text': text}, isDark),
            ),
          );
        }
      }
    ...
    if (widgets.isEmpty) {
      widgets.add(
        Text(
          'No additional AI narratives yet. Keep adding expenses to improve insights.',
          style: FintechTypography.bodyMedium.copyWith(
            color: isDark ? FintechColors.textSecondary : FintechColors.lightTextSecondary,
            fontStyle: FontStyle.italic,
          ),
  ...
  Widget _buildAdviceItem(dynamic advice, bool isDark) {
    ...
    final title = isMap
      ? (advice['title']?.toString() ?? advice['category']?.toString())
      : null;
    final message = isMap
      ? (advice['message']?.toString() ?? advice['reason']?.toString() ?? advice.toString())
      : advice.toString();
    final recommendation = isMap
      ? (advice['recommendation']?.toString() ?? advice['suggestion']?.toString())
      : null;
